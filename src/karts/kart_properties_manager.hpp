...
private:
    ...
    std::map<std::string, std::vector<size_t> > m_groups_2_indices;
...
public:
    ...
    size_t                   getKartId(const std::string &ident) const;
    ...
    std::vector<size_t>      getKartsInGroup        (const std::string& g);
